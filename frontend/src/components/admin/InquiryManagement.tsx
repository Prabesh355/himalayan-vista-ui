...
export const InquiryManagement = () => {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (response.trim()) {
        await adminService.respondToInquiry(
  ...
  const columns = [
    ...
    { key: "subject", header: "Subject" },
    {
      ...
      render: (item: InquiryItem) => (
        <span
          className={`px-2 py-1 rounded-full text-xs font-medium ${item.status === "replied" ? "bg-emerald-100 text-emerald-700" : item.status === "closed" ? "bg-slate-100 text-slate-700" : "bg-amber-100 text-amber-700"}`}
        >
          {item.status || "new"}
        </span>
      ),
    },
    {
      ...
      render: (item: InquiryItem) => (
        <span
          className={`px-2 py-1 rounded-full text-xs font-medium ${item.priority === "high" ? "bg-rose-100 text-rose-700" : item.priority === "low" ? "bg-sky-100 text-sky-700" : "bg-amber-100 text-amber-700"}`}
        >
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-xl bg-card border p-4">
          ...
          <p className="mt-2 text-3xl font-bold">
            {stats?.byStatus?.find((item) => item._id === "new")?.count ?? 0}
          </p>
        </div>
        <div className="rounded-xl bg-card border p-4">
          <p className="text-sm text-muted-foreground">Replied</p>
          <p className="mt-2 text-3xl font-bold">
            {stats?.byStatus?.find((item) => item._id === "replied")?.count ?? 0}
          </p>
        </div>
      </div>
      ...
      {(inquiriesQuery.error || statsQuery.error) && (
        <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-4 text-red-700">
          ...
          {(inquiriesQuery.error || statsQuery.error) instanceof Error
            ? (inquiriesQuery.error || statsQuery.error)?.message
            : "Please try again."}
        </div>
      )}
