...
export const BookingManagement = () => {
  ...
  const filteredData = useMemo(() => {
    return bookings.filter((booking) => {
      ...
      const packageTitle =
        typeof booking.package === "string" ? booking.package : booking.package?.title || "";
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Package",
      render: (item: BookingRow) =>
        typeof item.package === "string" ? item.package : item.package?.title || "Unknown",
    },
    {
      key: "travelDate",
      header: "Travel Date",
    ...
    <div className="space-y-6">
      ...
      {error && (
        <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-4 text-red-700">
          Unable to load bookings: {error instanceof Error ? error.message : "Please try again."}
        </div>
      )}
      ...
      <Dialog open={!!selectedBooking} onOpenChange={(open) => !open && setSelectedBooking(null)}>
        <DialogContent className="sm:max-w-[560px]">
          <DialogHeader>
            <DialogTitle>
              Booking Details - {selectedBooking?.bookingNumber || selectedBooking?._id}
            </DialogTitle>
          </DialogHeader>
          {selectedBooking && (
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-semibold">
                    {typeof selectedBooking.package === "string"
                      ? selectedBooking.package
                      : selectedBooking.package?.title || "Unknown"}
                  </p>
                </div>
                <div>
