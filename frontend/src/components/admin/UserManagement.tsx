...
export const UserManagement = () => {
  ...
  const filteredData = useMemo(() => {
    return users.filter((user) => {
      ...
      return (
        fullName.includes(term) || email.includes(term) || user.role.toLowerCase().includes(term)
      );
    });
  }, [searchTerm, users]);
  ...
  const columns = [
    ...
    { key: "email", header: "Email" },
    {
      ...
      render: (item: AdminUser) => (
        <span
          className={`flex items-center gap-1.5 ${item.role === "admin" ? "text-primary font-semibold" : "text-muted-foreground"}`}
        >
    ...
    },
    {
      ...
      render: (item: AdminUser) => (
        <span
          className={`px-2 py-1 rounded-full text-xs font-medium ${item.isActive === false ? "bg-rose-100 text-rose-700" : "bg-emerald-100 text-emerald-700"}`}
        >
    ...
    <div className="space-y-6">
      ...
      {error && (
        <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-4 text-red-700">
          Unable to load users: {error instanceof Error ? error.message : "Please try again."}
        </div>
      )}
