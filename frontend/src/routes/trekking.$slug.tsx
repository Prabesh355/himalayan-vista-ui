...
import { generateMeta, schemaBuilders, getPackageUrlPrefix } from "@/lib/seo";

export const Route = createFileRoute("/trekking/$slug")({
  loader: async ({ params }) => {
    try {
      const res = await api.get<{ success: boolean; data: any }>(`/packages/slug/${params.slug}`);
      return res.data?.data || null;
    } catch (err: any) {
      console.warn("Failed to fetch package in trekking loader:", err);
  ...
  head: ({ loaderData }) => {
    ...
    if (!pkg) {
      return generateMeta({
        title: "Trek Not Found — Nomads Navigate Nepal",
        description: "The requested trekking package was not found."
      });
    }

    const title = pkg.seoTitle || `${pkg.title} | ${pkg.duration?.days || 0} Days Trekking Package in Nepal`;
    const description = pkg.seoDescription || pkg.description || "";
    const keywords = pkg.keywords || `${pkg.title}, ${pkg.destination}, Nepal Trekking, travel`;
