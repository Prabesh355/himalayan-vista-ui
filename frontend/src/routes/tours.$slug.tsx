...
export const Route = createFileRoute("/tours/$slug")({
  loader: async ({ params }) => {
    try {
      ...
      console.warn("Failed to fetch package in tours loader:", err);
  ...
  head: ({ loaderData }) => {
    ...
    if (!pkg) {
      return generateMeta({
        title: "Tour Not Found — Nomads Navigate Nepal",
        description: "The requested tour package was not found."
      });
    }

    const title = pkg.seoTitle || `${pkg.title} | ${pkg.duration?.days || 0} Days Tour in Nepal`;
    const description = pkg.seoDescription || pkg.description || "";
    const keywords = pkg.keywords || `${pkg.title}, ${pkg.destination}, Nepal Tour, travel`;
