...
<<<<<<< HEAD
    ...
    return next(
      new AppError(`Validation failed: ${errorMessages.map((e) => e.message).join(", ")}`, 400),
    );
=======
    const errorMessages = errors.array().map(err => ({
      field: err.param,
...
<<<<<<< HEAD
  ...
  body("duration.days").optional().isInt({ min: 1 }).withMessage("Duration must be at least 1 day"),

  body("duration.nights").optional().isInt({ min: 0 }).withMessage("Nights cannot be negative"),
