...
function extractItinerary(markdown) {
  ...
  const startDay = (day, title) => {
    pushDay();
    currentDay = { day, title, detailParts: [], meta: [], highlights: [], activeSection: null };
  };
  ...
  for (const line of lines) {
    ...
    if (numberedDay) {
      ...
      if (numberedDay[3]) currentDay.detailParts.push(clean(numberedDay[3]));
      continue;
    }
    ...
    if (bulletDay) {
      ...
      if (bulletDay[3]) currentDay.detailParts.push(clean(bulletDay[3]));
      continue;
    }
    ...
    if (currentDay) {
      ...
      if (/^\*\*(Accommodation|Duration)\:/i.test(line)) {
        currentDay.meta.push(clean(line.replace(/^\*\*/, "").replace(/\*\*$/, "")));
        continue;
      }
      if (/^[-*•]\s+/.test(line)) {
  ...
  return {
    ...
    days: days.map((d) => ({
      day: d.day,
      title: d.title,
      detail: d.detailParts.join(" ").replace(/\s+/g, " ").trim(),
      meta: Array.from(new Set(d.meta)).filter(Boolean),
      highlights: Array.from(new Set(d.highlights)).filter(Boolean),
    })),
  };
}
...
for (const s of slugs) {
  ...
  console.log("\n---", s, "---");
  if (!block) {
    console.log("No itinerary block found");
    continue;
  }
