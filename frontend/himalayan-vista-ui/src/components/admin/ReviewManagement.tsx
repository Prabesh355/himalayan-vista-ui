  ...
  const columns = [
    {
      key: 'reviewer',
      header: 'Reviewer',
      render: (item: ReviewItem) => `${item.user?.firstName || 'Guest'} ${item.user?.lastName || ''}`.trim(),
    },
    {
      key: 'package',
      header: 'Package',
    ...
    },
    {
      ...
      header: 'Status',
      render: (item: ReviewItem) => (
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${item.status === 'approved' ? 'bg-emerald-100 text-emerald-700' : item.status === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-700'}`}>
          {item.status || 'pending'}
        </span>
      ),
    },
    ...
    <div className="space-y-6">
      ...
      {(reviewsQuery.error) && (
        <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-4 text-red-700">
          Unable to load reviews: {reviewsQuery.error instanceof Error ? reviewsQuery.error.message : 'Please try again.'}
        </div>
      )}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <DataTable
          data={filteredData}
          columns={columns}
          keyExtractor={(item) => item._id || item.id || `${item.rating}-${item.comment?.slice(0, 10)}`}
            ...
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-semibold">{selectedReview.user?.firstName || 'Guest'} {selectedReview.user?.lastName || ''}</p>
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{selectedReview.package?.destination || '—'}</p>
                </div>
                <div>
