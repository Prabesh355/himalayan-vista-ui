...
import { Users, DollarSign, Package, CalendarDays, ArrowUpRight, ArrowDownRight, MessageSquare, ShieldCheck } from 'lucide-react';
...
export const DashboardOverview = () => {
  ...
  const stats: StatusCard[] = [
    { label: 'Total Users', value: String(data?.dashboardStats.totalUsers ?? 0), change: 'Live', positive: true, icon: Users },
    { label: 'Active Packages', value: String(data?.dashboardStats.activePackages ?? 0), change: 'Live', positive: true, icon: Package },
    { label: 'Total Bookings', value: String(data?.dashboardStats.totalBookings ?? 0), change: 'Live', positive: true, icon: CalendarDays },
    { label: 'New Inquiries', value: String(data?.dashboardStats.newInquiries ?? 0), change: 'Live', positive: true, icon: MessageSquare },
    { label: 'Pending Reviews', value: String(data?.dashboardStats.pendingReviews ?? 0), change: 'Live', positive: true, icon: ShieldCheck },
    { label: 'Revenue Total', value: `$${totalRevenue.toLocaleString()}`, change: 'From booking stats', positive: true, icon: DollarSign },
  ];

  if (isLoading) {
    return <div className="rounded-2xl glass border border-border/50 p-10 text-center text-muted-foreground">Loading dashboard overview…</div>;
  }
  ...
  if (error) {
    return (
      <div className="rounded-2xl glass border border-red-500/20 bg-red-500/10 p-6 text-red-800">
        ...
        <p className="text-sm mt-2">{error instanceof Error ? error.message : 'Please try again.'}</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-7 gap-6">
        ...
        >
          <div className="mb-6">
            ...
            <p className="text-sm text-muted-foreground">Count and revenue grouped by booking status.</p>
          </div>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={bookingChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                />
                <Area type="monotone" dataKey="revenue" stroke="#0ea5e9" strokeWidth={3} fillOpacity={1} fill="url(#colorTotal)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
        ...
        >
          <div className="mb-6">
            ...
            <p className="text-sm text-muted-foreground">Breakdown of payment status values from live bookings.</p>
          </div>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
